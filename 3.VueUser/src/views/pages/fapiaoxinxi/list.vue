<template>
	<div class="app-contain" :style='{"padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.nongminxingming" placeholder="开票名称"
						clearable>
					</el-input>
				</div>
			</div>
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('fapiaoxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="发票号码" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.fapiaohaoma}}
							</template>
						</el-table-column>
						<el-table-column label="开票日期" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kaipiaoriqi}}
							</template>
						</el-table-column>
						<el-table-column label="购买账号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.nongminzhanghao}}
							</template>
						</el-table-column>
						<el-table-column label="开票名称" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.nongminxingming}}
							</template>
						</el-table-column>
						...
						<el-table-column label="含税单价" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.hanshuidanjia}}
							</template>
						</el-table-column>
						<el-table-column label="税率" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.shuilv}}
							</template>
						</el-table-column>
						<el-table-column label="开票金额" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kaipiaojine}}
							</template>
						</el-table-column>
						...
						<el-table-column label="开票企业名称" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kaipiaoqiyemingcheng}}
							</template>
						</el-table-column>
					</el-table>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/fapiaoxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if(searchQuery.value.nongminxingming&&searchQuery.value.nongminxingming!=''){
			params.nongminxingming = '%' + searchQuery.value.nongminxingming + '%'
		}
		context?.$http({
	...
	const tableDetailClick = (row) => {
		router.push('fapiaoxinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
	...
	.list_search {
		...
		.search_btn_view {
			...
			.search_btn {
				...
				margin: 0 10px 0 0;
				color: #fff;
				background: url(http://clfile.zggen.cn/20240307/60098c68ab4e4fc6b0309ba2ddb3f2fe.jpg) no-repeat center top / 100% 100%;
				width: auto;
				font-size: 14px;
				height: 40px;
			}
			...
			.add_btn {
				...
				margin: 0 10px 0 0;
				color: #fff;
				background: url(http://clfile.zggen.cn/20240307/c9a4f5edcbfb4628b5ee6143b5c10fad.jpg) no-repeat center top / 100% 100%;
				width: auto;
				font-size: 14px;
				height: 40px;
			}
