	...
	<div class="app-contain" :style='{"padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('shengzishangpin','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
				...
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						...
						<div class="data_content">
							<div class="data_title">{{item.shangpinmingcheng}}</div>
...
<script setup>
	...
	//基础信息
	const tableName = 'shengzishangpin'
	const formName = '生资商品'
	//基础信息
	const breadList = ref([{
		name: formName
	...
	const addClick = () => {
		router.push('/index/shengzishangpinAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.fenlei = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('shengzishangpinDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
