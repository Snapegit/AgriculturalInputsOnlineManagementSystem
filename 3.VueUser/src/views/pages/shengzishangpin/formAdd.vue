			...
			<el-row>
				...
				<el-col :span="24">
					<el-form-item label="分类" prop="fenlei">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.fenlei?true:false"
							v-model="form.fenlei" 
							placeholder="请选择分类"
							style="width:100%;"
							>
							<el-option v-for="(item,index) in fenleiLists" :label="item"
								:value="item"
								>
							</el-option>
						</el-select>
				...
				<el-col :span="24">
					<el-form-item label="参数" prop="canshu">
						<el-input v-model="form.canshu" placeholder="参数" type="textarea"
						:readonly="!isAdd||disabledForm.canshu?true:false"
						/>
					</el-form-item>
				</el-col>
				<el-col :span="24">
					<el-form-item label="商品详情" prop="shangpinxiangqing">
						<editor class="list_editor" :value="form.shangpinxiangqing" placeholder="请输入商品详情" :readonly="!isAdd||disabledForm.shangpinxiangqing?true:false"
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.shangpinxiangqing = res.data.data.shangpinxiangqing.replace(reg,'../../../cl84235399/file');
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='fenlei'){
					form.value.fenlei = row[x];
					disabledForm.value.fenlei = true;
					continue;
				}
				if(x=='guige'){
				...
				if(x=='canshu'){
					form.value.canshu = row[x];
					disabledForm.value.canshu = true;
					continue;
				}
		...
		context?.$http({
			...
			fenleiLists.value = res.data.data
		})
	}
	//初始化
	//取消
