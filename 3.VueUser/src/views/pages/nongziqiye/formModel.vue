	...
	<div class="app-contain" :style='{"padding":"20px 0px","margin":"20px auto 40px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">执照经营许可证</div>
					<div class="info_text">
						<el-button class="info_down" v-if="detail.zhizhaojingyingxukezheng" @click="downClick(detail.zhizhaojingyingxukezheng)">点击下载</el-button>
				...
				<div class="btn_view">
					<el-button class="approval_btn" v-if="btnAuth('nongziqiye','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('nongziqiye','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('nongziqiye','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('nongziqiye','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="企业简介" name="first">
				<div v-html="detail.qiyejianjie"></div>
			</el-tab-pane>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.qiyemingcheng
			bannerList.value = res.data.data.qiyezhaopian?res.data.data.qiyezhaopian.split(','):[]
