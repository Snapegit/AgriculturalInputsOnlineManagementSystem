	...
	<div class="app-contain" :style='{"padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('nongziqiye','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.qiyezhaopian && item.qiyezhaopian.substr(0,4)=='http'" @click.stop="preViewClick(item.qiyezhaopian)">
							<el-image class="data_img" :src="item.qiyezhaopian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.qiyezhaopian.split(',')[0])">
							<el-image class="data_img" :src="item.qiyezhaopian?$config.url + item.qiyezhaopian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title">{{item.qiyemingcheng}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/nongziqiyeAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('nongziqiyeDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
	...
	.list_bottom {
		...
		.data_box {
			...
			.data_view {
				...
				.data_item {
					cursor: pointer;
					border: 0px solid rgba(136, 179, 1,.3);
					padding: 10px;
					margin: 0 2% 24px 0;
