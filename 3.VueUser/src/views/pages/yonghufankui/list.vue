		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.fankuineirong" placeholder="反馈内容"
						clearable>
					</el-input>
				</div>
			</div>
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('yonghufankui','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="反馈时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.fankuishijian}}
							</template>
						</el-table-column>
						<el-table-column label="农民账号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.nongminzhanghao}}
							</template>
						</el-table-column>
						<el-table-column label="农民姓名" :resizable='true' align="left" header-align="left">
							<template #default="scope">
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/yonghufankuiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if(searchQuery.value.fankuineirong&&searchQuery.value.fankuineirong!=''){
			params.fankuineirong = '%' + searchQuery.value.fankuineirong + '%'
		}
		context?.$http({
	...
	const tableDetailClick = (row) => {
		router.push('yonghufankuiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
