<template>
	<div class="home">
		<div class="index_top">
			<div class="index_top_title">农场投入品运营线上管理系统</div>
		...
		<el-scrollbar class="contain_view">
			...
			<div class="rotation_view">
				<mySwiper :type="3" :data="rotationList" :autoHeight="false" :autoplay="true"
					...
					:freeMode="false" :effectType="6"
					:direction="horizontal">
					<template #default="scope">
						<img :style='{"border":"0px solid #fff","width":"100%","borderRadius":"4px","height":"400px"}' :src="scope.row.value?$config.url + scope.row.value:''">
					</template>
				</mySwiper>
			</div>
			<router-view />
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'nongmin'){
				context?.$toolUtil.storageSet("frontName", res.data.data.nongminzhanghao)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'nongmin'){
				context?.$toolUtil.storageSet('headportrait',res.data.data.touxiang)
			}
			if(context?.$toolUtil.storageGet('frontSessionTable') == 'nongziqiye'){
...
<style lang="scss" scoped>
	...
	.index_top {
		...
		overflow: hidden;
		color: #333;
		background: url(http://clfile.zggen.cn/20240307/41ec997c466645c6bf2905dcdcebb7de.png);
		display: flex;
		width: 100%;
	...
	.menu_scrollbar {
		...
		margin: 0px auto 0;
		z-index: 1000;
		background: url(http://clfile.zggen.cn/20240307/e3277d66e35e40cf8cfe0bf752046fa1.jpg) repeat-x center top / auto 100%;
		display: flex;
		width: 100%;
		border-color: #d7f08b;
	...
	.menu_view {
		...
		:deep(.is-active) {
			color: #fff !important;
			background: url(http://clfile.zggen.cn/20240307/34a4de51f7e8423e910d69f027bf0666.jpg) no-repeat center center / 100% 100% !important;
