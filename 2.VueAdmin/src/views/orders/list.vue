...
<template>
	<div>
		<div class="app-contain">
			...
			<div class="list_search_view">
				...
				<div class="btn_view">
					...
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('orders'+(orderStatus?'/'+orderStatus:''),'月营业额')">月营业额</el-button>
					<el-button type="warning" @click="echartClick2" v-if="btnAuth('orders'+(orderStatus?'/'+orderStatus:''),'商品销量')">商品销量</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column prop="addtime" label="下单时间" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						{{scope.row.addtime}}
					</template>
				</el-table-column>
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'月营业额'" width="70%">
			<el-tabs v-model="echartActive" class="demo-tabs" @tab-change="echartTabClick" type="card">
			    <el-tab-pane label="月营业额" name="1"></el-tab-pane>
			    <el-tab-pane label="商品销量" name="2"></el-tab-pane>
			</el-tabs>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var totalEchart1 = echarts.init(document.getElementById("totalEchart1"),'macarons');
			context?.$http({
	...
	const echartClick2 = ()=>{
		...
		echartVisible.value = true
		nextTick(()=>{
			var buynumberEchart2 = echarts.init(document.getElementById("buynumberEchart2"),'macarons');
			context?.$http({
