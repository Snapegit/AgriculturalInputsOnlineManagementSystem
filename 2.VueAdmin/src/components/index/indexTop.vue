<template>
	<div class="top_view">
		...
		<div class="projectTitle">农场投入品运营线上管理系统</div>
...
<script setup>
	...
	const getSession = () => {
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('sessionTable') == 'nongmin'){
				context?.$toolUtil.storageSet('adminHeadportrait',res.data.data.touxiang)
			}
			if(context?.$toolUtil.storageGet('sessionTable') == 'nongziqiye'){
				context?.$toolUtil.storageSet('adminHeadportrait',res.data.data.qiyezhaopian)
			}
		})
	}
	// 退出登录
